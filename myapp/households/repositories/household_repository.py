...
class IHouseholdRepository(ABC):
    @abstractmethod
    def list_all(self, status: str | None = None) -> Iterable[Household]: ...
    ...
    def get_by_apartment_block(
        self, apartment: str, block: str
    ) -> Household | None: ...

    @abstractmethod
    def search(self, apartment: str | None, block: str | None) -> Iterable[Household]: ...
...
class DjangoHouseholdRepository(IHouseholdRepository):
    ...
    def get_by_apartment_block(self, apartment, block):
        return Household.objects.filter(apartment=apartment, block=block).first()

    def search(self, apartment, block):
        qs = Household.objects.exclude(status=Household.Status.ARCHIVED)
        if apartment:
