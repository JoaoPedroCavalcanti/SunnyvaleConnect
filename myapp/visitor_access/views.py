...
from sunnyValeConnect.utils.send_email_to_visitor import (
...
class VisitorAccessViewSet(ModelViewSet):
    ...
    permission_classes = [
        IsAuthenticatedOrCheckInAndCheckOut,
    ]
    ...
    def get_queryset(self):
        ...
        return VisitorAccessModel.objects.filter(host_user=self.request.user).order_by(
            "-scheduled_date"
        )
    ...
    def checkin(self, request, *args, **kwargs):
        obj_id = unmix_strings(
            kwargs.get("visitor_access_link_checkin"), secret_mixin_string
        )
        ...
        if (
            obj.checkin_date_time < timezone.now()
            and timezone.now() < obj.checkout_date_time
        ):
            if obj.checkin_code != "":
            ...
            send_checkin_notification(
                to_email=user_email, user_name=user_name, visitor_name=visitor_name
            )
    ...
    def checkout(self, request, *args, **kwargs):
        obj_id = unmix_strings(
            kwargs.get("visitor_access_link_checkout"), secret_mixin_string
        )
        obj = get_object_or_404(VisitorAccessModel, id=obj_id)
