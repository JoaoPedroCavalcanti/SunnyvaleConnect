...
from sunnyValeConnect.utils.settings_config import (
    base_url_visitor_access,
    secret_mixin_string,
)
...
class VisitorAccessSerializer(ModelSerializer):
        ...
        if self.user.is_staff:
            if attrs.get("host_user") == None:
                raise ValidationError(
                    {"host_user": "This field is required for staff users."}
                )
            return attrs
        if attrs.get("host_user"):
            raise ValidationError(
                {"host_user": "This field is automatically set to the current user."}
            )
    ...
    def validate_scheduled_date(self, value):
        if value < timezone.now():
            raise ValidationError(
                {
                    "Scheduled_date": "You can not create a visitor access with a past date."
                }
            )
        return value
    ...
    def create(self, validated_data):
        ...
        if instance.checkout_date_time == None:
            instance.checkout_date_time = instance.checkin_date_time + timedelta(
                hours=3
            )
        # Gere o link_checkin após a criação do objeto (e ai posso acessar o id)
        mixed_string = mix_strings(
            string=str(instance.id), mix_code=secret_mixin_string
        )
