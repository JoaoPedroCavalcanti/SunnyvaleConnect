...
@pytest.fixture
def service():
    return BBQReservationService(repository=FakeBBQRepository())
...
class TestCreate:
    def test_regular_user_creates_for_self(self, service):
        u = _user()
        item = service.create(u, {"reservation_date": _future()})
        assert item.reservation_user is u

    def test_regular_user_cannot_pass_reservation_user(self, service):
        u = _user(1)
        with pytest.raises(BusinessRuleError):
            service.create(u, {"reservation_date": _future(), "reservation_user": _user(2)})

    def test_admin_must_pass_reservation_user(self, service):
        with pytest.raises(BusinessRuleError):
            service.create(_user(is_staff=True), {"reservation_date": _future()})

    def test_admin_creates_for_other(self, service):
        target = _user(2)
        item = service.create(
    ...
    def test_date_collision(self, service):
        d = _future()
        service.create(_user(1), {"reservation_date": d})
        with pytest.raises(BusinessRuleError):
            service.create(_user(2), {"reservation_date": d})

    def test_30_day_window_for_regular_user(self, service):
        d1 = _future(5)
        d2 = _future(15)
        service.create(_user(1), {"reservation_date": d1})
        with pytest.raises(BusinessRuleError):
            service.create(_user(1), {"reservation_date": d2})

    def test_admin_bypasses_30_day_window(self, service):
        admin = _user(99, is_staff=True)
        target = _user(1)
        service.create(admin, {"reservation_date": _future(5), "reservation_user": target})
        # second booking within 30 days from admin: allowed
        item = service.create(
            admin, {"reservation_date": _future(15), "reservation_user": target}
        )
