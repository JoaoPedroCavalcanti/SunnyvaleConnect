...
def service_requests_list_and_create(request):
    ...
    def get_user_object_or_all_objects():
        ...
        if request.user.is_staff:
            all_objects = ServiceRequestModel.objects.all().order_by('-request_scheduled_date')
            return all_objects
        
        all_objects = ServiceRequestModel.objects.filter(requester_user_id = request.user).order_by('-request_scheduled_date')
        return all_objects
...
def service_request_detail_retrieve_and_delete(request, pk):
    ...
    if request.method == 'GET':
        ...
        if service_request.requester_user_id != request.user.id and not request.user.is_staff:
            return Response({"detail": "No Service Request matches the given query."}, status = status.HTTP_404_NOT_FOUND)
...
def accept_request(request, pk, accept_or_decline):
    ...
    return Response({"Status": f'Service Request from {service_request.requester_user} was {service_request.status}',
                     "Details": request.data}, status=status.HTTP_200_OK)