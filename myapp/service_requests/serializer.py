...
class ServiceRequestSerializer(serializers.Serializer):
    ...
    service_type = serializers.CharField(max_length=150, required=False, allow_blank=True, default="Other")
    ...
    responsable_staff = serializers.CharField(max_length=50, required=False, allow_blank=True)
    scheduled_date = serializers.DateTimeField(required=False, allow_null=True)
    more_details = serializers.CharField(max_length=200, required=False, allow_blank=True)
    ...
    def validate_scheduled_date(self, value):
        if value is not None and not isinstance(value, datetime):
            raise ValidationError("O valor deve estar no formato de data e hora correto.")
        return value
